...
ArrayList<Bend> bends2 = new ArrayList<Bend>();
void setup() {
  size(500, 500, P3D);
...
void keyPressed() { //按按鍵，可調整 theta0 預設角度
  if(key=='-' && bends2.get(0).theta0 > 0.1) { //按'0'，會減少角度
    bends2.get(0).theta0 -= 0.1; //修改bending 的預設角度
  }else if(bends2.get(0).theta0 < PI - 0.1) { //其他鍵，都會增加角度
    bends2.get(0).theta0 += 0.1; //修改bending 的預設角度
  }
}
...
void mouseDragged() { //拖曳上方的頂點，並即時模擬
  if(mouseButton==RIGHT) { // 右鍵「左右拖曳」移動（上方）頂點位置
    ...
    particles.get(5).x = PVector.add(particles.get(3).x, v);
    particles.get(5).p = PVector.add(particles.get(3).x, v);
    particles.get(5).locked = true; //先固定、鎖住下方的點，讓它不要模擬
    particles.get(5).w = 0; //先固定、鎖住下方的點，讓它不要模擬
  }
}
...
void mouseReleased() { //放開時，再解鎖下方的點，讓它可以模擬
  ...
  particles.get(2).w = 1;
  particles.get(5).locked = false;
  particles.get(5).w = 1;
}

void generateTwoRectangle() { //要處理「不能轉」及「能轉」的Hindge
  Particle p1 = new Particle(-100, -100); //左上，z座標都會放亂數
  Particle p2 = new Particle(-100, 0); //左
  Particle p3 = new Particle(-100, +100); //左下
  Particle p4 = new Particle(100, -100); //右上
  Particle p5 = new Particle(100, 0); //右
  Particle p6 = new Particle(100, +100); //右下
  p1.locked = true; //固定以便觀察
  p1.w = 0; // 也要設成0 才會在 projectConstraint() 時真固定
  ...
  bends.add(new Bend(p2,p4,p1,p5)); //平面
  bends.add(new Bend(p3,p5,p2,p6)); //平面
  bends2.add(new Bend(p2,p5,p4,p3)); //可彎曲的面
  bends2.get(0).theta0 -= 0.2;
  for( Triangle t : triangles ) {
    sticks.add(new Stick(t.p[0], t.p[1]));
    sticks.add(new Stick(t.p[1], t.p[2]));
    sticks.add(new Stick(t.p[2], t.p[0]));
  }
  for( Stick s : sticks ) {
