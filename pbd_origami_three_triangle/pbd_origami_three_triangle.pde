...
void keyPressed() { //按按鍵，可調整 theta0 預設角度
  if(key=='-' && bends.get(0).theta0 > 0.1 && bends.get(1).theta0 >0.1) { //按'0'，會減少角度
    bends.get(0).theta0 -= 0.1; //修改bending 的預設角度
    bends.get(1).theta0 -= 0.1;
  }else if(bends.get(0).theta0 < PI - 0.1 && bends.get(1).theta0 < PI - 0.1) { //其他鍵，都會增加角度
    bends.get(0).theta0 += 0.1; //修改bending 的預設角度
    bends.get(1).theta0 += 0.1;
  }
}
...
void mouseDragged() { //拖曳上方的頂點，並即時模擬
  if(mouseButton==RIGHT) { // 右鍵「左右拖曳」移動（上方）頂點位置
    ...
    PVector p0 = particles.get(2).x, p1 = particles.get(4).x;
    float len = dist(p0.x, p0.y, p1.x, p1.y);
    float diff = mouseY - pmouseY;
    PVector v = PVector.sub(p1,p0).mult((len+diff)/len); //增加長度
    particles.get(4).x = PVector.add(p0, v);
    particles.get(4).p = PVector.add(p0, v);
    particles.get(4).locked = true; //先固定、鎖住下方的點，讓它不要模擬
    particles.get(4).w = 0; //先固定、鎖住下方的點，讓它不要模擬
  }
}

void mouseReleased() { //放開時，再解鎖下方的點，讓它可以模擬
  particles.get(4).locked = false;
  particles.get(4).w = 1;
}
...
void generateTwoTriangle() {
  ...
  Particle p5 = new Particle(-50, 250, 0); //新的點
  ...
  sticks.add(new Stick(p4,p5)); //新的點
  sticks.add(new Stick(p1,p5)); //新的點
  for( Stick s : sticks ) {
    stretches.add(new Stretch(s));
  }
  triangles.add(new Triangle(p1,p2,p3));
  triangles.add(new Triangle(p1,p2,p4));
  triangles.add(new Triangle(p1,p4,p5));
  bends.add(new Bend(p1,p2,p3,p4));
  bends.add(new Bend(p1,p2,p4,p5));
